        ...
        <DialogBody>
          <form onSubmit={(e) => {e.preventDefault(); createBankAccout(form)}} className="space-y-4" action="/api/profile/bank/create" method="post">
            ...
            <div id="account-owner-name-input-container" className="flex flex-col w-full space-y-1">
              ...
              <input onChange={e => {setForm({...form, name: e.target.value})}} type="text" className="p-2 h-10 border rounded-lg border-gray-400 focus:border-none focus:border-white"/>
            </div>
            <div id="account-number-input-container" className="flex flex-col w-full space-y-1">
              <label htmlFor="">Account Number</label>
              <input onChange={e => {setForm({...form, number: e.target.value})}} type="text" className="p-2 h-10 border rounded-lg border-gray-400 focus:border-none focus:border-white"/>
            </div>
